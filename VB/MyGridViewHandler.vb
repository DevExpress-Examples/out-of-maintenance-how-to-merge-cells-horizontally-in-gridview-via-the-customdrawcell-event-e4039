	...
	Public Class MyGridViewHandler
		...
		Private Sub view_CustomDrawCell(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs)
			...
			If currentInfo IsNot Nothing Then
				...
				Dim currentClip As RectangleF = e.Graphics.ClipBounds
				If currentInfo IsNot Nothing Then
					...
					clipBoundsX = If(CInt(Fix(currentClip.X)) < e.Bounds.X, e.Bounds.X - 4, CInt(Fix(currentClip.X)))

					If View.LeftCoord > 0 Then
						e.Cache.ClipInfo.SetClip(New Rectangle(clipBoundsX, CInt(Fix(currentClip.Y)), textRect.Width, CInt(Fix(currentClip.Height))))
					End If
				...
				e.Cache.ClipInfo.SetClip(New Rectangle(CInt(Fix(currentClip.X)), CInt(Fix(currentClip.Y)), CInt(Fix(currentClip.Width)), CInt(Fix(currentClip.Height))))
			End If
		End Sub
	End Class
